...
## A closer look at the `my_vlpbek_data` `data.frame`
...
## Making a `my_flow_basics` object
...
## A closer look at the `my_flow_basics` object

The `my_flow_basics` object is a list containing `r length(my_flow_basics)` elements:  
...
## Making a `flow_insights` object 

```{r}
# make the actual my_vlpbek_compact object from the my_flow_basics object
my_flow_insights <- make_flow_insights(my_flow_basics)
```

## A closer look at the `my_flow_insights` object 

The `my_flow_insights` object is a list containing `r length(my_flow_insights)` elements:  
